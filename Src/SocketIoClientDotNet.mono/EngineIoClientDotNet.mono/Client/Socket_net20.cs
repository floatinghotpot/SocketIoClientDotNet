...
//            EventTasks.Exec((n) =>
        ...
        {
            var log = LogManager.GetLogger(Global.CallerName());
		            log.Info(string.Format("ReadyState={0} Transport.Writeable={1} Upgrading={2} WriteBuffer.Count={3}",ReadyState,mTransport.Writable,Upgrading, WriteBuffer.Count));
            if (ReadyState != ReadyStateEnum.CLOSED && this.mTransport.Writable && !Upgrading && WriteBuffer.Count != 0)
            {
                log.Info(string.Format("Flush {0} packets in socket", WriteBuffer.Count));
                PrevBufferLen = WriteBuffer.Count;
                mTransport.Send(WriteBuffer);
                Emit(EVENT_FLUSH);
                return true;
            }
            else
        ...
        private void OnHandshake(HandshakeData handshakeData)
        {
            ...
            mTransport.Query.Add("sid", handshakeData.Sid);
        ...
        private void SetPing()
        {
            ...
            {
				var log = LogManager.GetLogger(Global.CallerName());
				log.Info(string.Format("writing ping packet - expecting pong within {0}ms, ReadyState = {1}", PingTimeout, ReadyState));
        ...
        private void OnOpen()
        {
            ...
            PriorWebsocketSuccess = WebSocket.NAME == mTransport.Name;

            Flush();
            Emit(EVENT_OPEN);


            if (ReadyState == ReadyStateEnum.OPEN && Upgrade && mTransport is Polling)
            ...
            private class ProbeEventPacketListener : IListener
            {
                ...
                void IListener.Call(params object[] args)
                {
                    ...
                    if (Packet.PONG == msg.Type && "probe" == (string) msg.Data)
                    {
                        ...
                        ((Polling) _onTransportOpenListener.Parameters.Socket.mTransport).Pause(
                            () =>
                            {
                                if (_onTransportOpenListener.Parameters.Failed[0])
                                {
        ...
        private void OnClose(string reason, Exception desc = null)
        {
            if (this.ReadyState == ReadyStateEnum.OPENING || this.ReadyState == ReadyStateEnum.OPEN)
            {
                ...
                if (this.mTransport != null)
                {
                    ...
                    this.mTransport.Off(EVENT_CLOSE);
				...
				log.Info("Socket.ReadyState = CLOSE");
        ...
        internal void OnHeartbeat(long timeout)
        {
            ...
            PingTimeoutTimer = EasyTimer.SetTimeout(() =>
            {
				...
				if (ReadyState == ReadyStateEnum.CLOSED) return;
